...
if [ -z "${BACKHAUL_PIPE_BOOTSTRAP:-}" ] && [ ! -t 0 ]; then
    tmp="$(mktemp)"
    cat > "$tmp"
    chmod +x "$tmp"
    exec env BACKHAUL_PIPE_BOOTSTRAP=1 bash "$tmp" "$@"
fi
...
DEFAULT_MIRROR_BASE="http://62.60.223.120:8002"
MIRROR_BASE="${MIRROR_BASE_URL:-$DEFAULT_MIRROR_BASE}"
if [[ "$MIRROR_BASE" == __*__ ]]; then
    echo "Mirror base not detected. Please download from the mirror host or set MIRROR_BASE_URL."
    exit 1
fi
DOWNLOAD_BASE="${MIRROR_DOWNLOAD_BASE:-${MIRROR_BASE%/}/downloads}"

download_file() {
    local url="$1"
    local dest="$2"
    if command -v curl >/dev/null 2>&1; then
        curl -fsSL --connect-timeout 10 --retry 3 -o "$dest" "$url" || return 1
        return 0
    fi
    if command -v wget >/dev/null 2>&1; then
        wget --inet4-only -O "$dest" "$url" || return 1
        return 0
    fi
    echo "Missing curl or wget to download ${url}"
    return 1
}
...
install_packages() {
    ...
    for pkg in "${packages[@]}"; do
        if command -v "$pkg" >/dev/null 2>&1; then
            colorize green "${pkg} already installed, skipping download."
        else
            missing+=("$pkg")
        fi
    done
    ...
    ubuntu | debian | armbian)
        local release_dir="${release_version//./}"
        case "$release_dir" in
        2404|2410|240*|24) release_dir="2404" ;;
        2204|2210|22*) release_dir="2204" ;;
        *) release_dir="2204" ;;
        esac
        ...
        *)
            colorize red "Unsupported deb architecture for base packages."
            exit 1
            ;;
        esac

        for pkg in "${missing[@]}"; do
            local pkg_url="${DOWNLOAD_BASE}/${pkg}/ubuntu/${release_dir}/${pkg}_${deb_arch}.deb"
            local pkg_dest="/tmp/${pkg}.deb"
            if ! download_file "$pkg_url" "$pkg_dest"; then
                colorize red "Failed to download ${pkg} from mirror: ${pkg_url}"
                press_key
                exit 1
            fi
            dpkg -i "$pkg_dest"
    ...
    *)
        if command -v apt-get >/dev/null 2>&1; then
            ...
            colorize red "Error: Unsupported package manager. Please install ${missing[*]} manually."
            press_key
            exit 1
        fi
        ;;
...
download_and_extract_backhaul() {
    ...
    case "$ARCH" in
        x86_64)
            DOWNLOAD_URL="${DOWNLOAD_BASE}/backhaul/backhaul_linux_amd64.tar.gz"
            ;;
        arm64|aarch64)
            DOWNLOAD_URL="${DOWNLOAD_BASE}/backhaul/backhaul_linux_arm64.tar.gz"
            ;;
        *)
    ...
    sleep 1
    if ! download_file "$DOWNLOAD_URL" "$DOWNLOAD_DIR/backhaul.tar.gz"; then
        echo -e "${RED}Failed to download Backhaul core from mirror: $DOWNLOAD_URL${NC}"
        sleep 1
        exit 1
    fi
    ...
    elif [[ -f "${config_dir}/backhaul" ]]; then
        mv "${config_dir}/backhaul" "${config_dir}/backhaul_premium"
        chmod u+x "${config_dir}/backhaul_premium"
    else
        echo -e "${RED}Backhaul core binary not found after extraction.${NC}"
        rm -rf "$DOWNLOAD_DIR"
        exit 1
    fi
...
SCRIPT_URL="${MIRROR_BASE%/}/install/backhaul/backhaul.sh"
...
download_file "$SCRIPT_URL" "$DEST_DIR/$BACKHAUL_SCRIPT"
